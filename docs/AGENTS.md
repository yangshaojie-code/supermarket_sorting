...
更新：2026-08-30。当前比赛 **DG-202606 智慧零售**，初赛提交 **2026-09-15**。
...
| [DG-202606-A-数据采集与YOLO.md](./DG-202606-A-数据采集与YOLO.md) | A 线：采图脚本、仿真标注、YOLOv8s、交付权重 | 编排/限幅/`client_task_1.py` |
...
已在仿真跑过：预检（关节/odom/相机/KDL/RGB-D/雷达/任务 JSON）；P2 干跑 1× `kele` → `DONE`（固定 Baseline `count:1` 正常）；P3 代码 + `kele.pt` 可在 cuda 加载。

未做：9 类数据集与训练；底盘去货架；雷达绕障；真抓真放；5 单随机；评分主入口。

下一步：A 写采图；B 把车开到拣货区让 P3 非空，并用可乐打通一次取送。操作细节以 [AGENT-BRIEF](./DG-202606-AGENT-BRIEF.md) 为准。
